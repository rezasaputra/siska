...
class U_pegawai extends CI_Controller {
    ...
    $data['msg']	=$msg;
		...
		$data['lihat']=$this->Madmin->tampil_data_field('user','jenis_user',2);
  ...
  function change_status($tabel="",$id="", $value='')
	...
	$user=$this->Madmin->tampil_data_field('user','jenis_user',2);
...
//user
    function tambah_user(jenis)
      {
          $.ajax({
              url : "'.base_url().'siska/U_pegawai/tambah_user/user/"+jenis,
              type : "POST",
              data : {"nama":$("#nama").val(),},
      ...
      function edit_user(id)
        {
            ...
            $.ajax({
                url : "'.base_url().'siska/U_pegawai/tampil_user_edit/user",
                type : "GET",
                data : data,
                success: function(data){
                    $(window).scrollTop(0);
        ...
        {
          $.ajax({
                url : "'.base_url().'siska/U_pegawai/tabel_user/user	",
                success: function(data){
        ...
        function aktif(id,value)
          {
            $.ajax({
               url : "'.base_url().'siska/U_pegawai/change_status/'.$tabel.'/"+id+"/"+value,
               success: function(data){
                 get_tabel_user();
               }
           });
        ...
        function confirm_del(id)
          {
              bootbox.confirm({
                  ...
                  callback: function(result){
                     if (result)
                     {
                         document.location.href = "'.base_url().'siska/U_pegawai/delete_user/'.$tabel.'/"+id;
                     }
                  }
              })
          }';
